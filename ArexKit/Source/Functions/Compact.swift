...
*/
public func compact<T>(value: T?) -> [T] {
    if let value = value {
        return [value]
    } else {
        return []
...
/**
    ...
    or zero elements (if value is `nil`).
*/
public func compact<T>(value: T?) -> ContiguousArray<T> {
    if let value = value {
        return [value]
    } else {
        return []
...
/**
    ...
    or zero elements (if value is `nil`).
*/
public func compact<T>(value: T?) -> ArraySlice<T> {
    if let value = value {
        return [value]
    } else {
        return []
...
*/
public func compact<T>(source: [T?]) -> [T] {
...
*/
public func compact<T>(source: ContiguousArray<T?>) -> ContiguousArray<T> {
    return source.flatMap(compact)
}
...
/**
    ...
    :returns: An `ArraySlice` containing the non-`nil` elements of `source`.
*/
public func compact<T>(source: ArraySlice<T?>) -> ArraySlice<T> {
    return source.flatMap(compact)
}
...
*/
public func compact<S: SequenceType, T where S.Generator.Element == T?>(source: S) -> [T] {
...
*/
public func compact<C: CollectionType, T where C.Generator.Element == T?>(source: C) -> [T] {
    return flatMap(source, compact)
}
